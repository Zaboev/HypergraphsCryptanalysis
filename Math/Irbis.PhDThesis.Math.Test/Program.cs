...
using Irbis.PhDThesis.Math.Cryptanalysis;
...
using Irbis.PhDThesis.Math.HypergraphSynthesis;
using Irbis.PhDThesis.Math.Cryptanalysis.KnownPlaintext;
...
void TestAttack()
{
    int verticesCount = 128;
    int simplicesDimension = 3;
    int smallBlockSizeBytes = 2; // например: 2 байта на вершину

    var key = new HypergraphBuilder().Randomize(verticesCount, simplicesDimension, 12345);
    AttackDemo.Run(key, smallBlockSizeBytes);
}

void TestKnownPlaintextAttack()
{
    int n = 128;
    int k = 3;
    int smallBlockSizeBytes = 2;

    var key = new HypergraphBuilder().Randomize(n, k, seed: 12345);

    // 1) "перехватываем" пары (в тесте генерируем сами)
    var pairs = PairsGenerator.Generate(key, smallBlockSizeBytes, pairsCount: 400, seed: 999);

    // 2) восстанавливаем модель M, Minv, c
    ...
    var target = pairs[10]; // выбрали любую пару для проверки
    var recovered = hacked.Decrypt(target.Ciphertext);

    Console.WriteLine("Recovered OK: " + recovered.AsSpan().SequenceEqual(target.Plaintext));
}
